...
public class CrudWithMongo {
    ...
    public static void updateStud(String _id, String studid ,String studname){
        try (MongoClient mc = MongoClients.create(CONNECTION_STRING)) {
            MongoDatabase db = mc.getDatabase(DATABASE_NAME);
            MongoCollection<Document> col = db.getCollection(COLLECTION_NAME);
    ...
    public static void main (String [ ] args){
        //callback method / calling method
            createStud("5", "Suresh", "JAVA");
            readStud();
            createStud("4", "Pavan", "React");
            readStud();
            updateStud("5", "SAM", "HTML");
            readStud();
            deleteStud("4");
            readStud();
    }
